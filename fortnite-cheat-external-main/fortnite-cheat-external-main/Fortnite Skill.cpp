...
// thanks to all of my nigga who will paste this source , sellers are bigs retarded monkeys <3
...
#define SWORD 0x0
#define Bonetoword 0x95ecb60
#define unword 0x95ecb60
#define sword_bone 0x95ecb60
#define compnenttoword 0x95ecb60
#define OFFSET_PLAYERCONTROLLER 0x30 
#define OFFSET_TEAMINDEX 0xED0 
...
int main(int argc, const char* argv[])
{
	...
	while (hwnd == NULL)
	{
		SetConsoleTitle("AstralCheats [private]");
		printf("Injecting...");
		Sleep(1000);
		system("cls");
		XorS(wind, "Fortnite  ");
	...
	XorS(base, "Injected! (DONT CLOSE THIS WINDOW) %p.\n");
...
void render() {
	...
	if (ShowMenu)
	{
		...
		XorS(menu, "AstralCheats [PRIVATE EXTERNAL]");
		...
		ImGui::Begin(menu.decrypt(), &ShowMenu, ImVec2(500, 470), 0.99f, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse);
		...
		ImGui::SliderInt("", &VisDist, 50, 250);
		ImGui::Separator();
		...
		ImGui::SliderFloat(aim6.decrypt(), &AimFOV, 250.f, 950.f);
		ImGui::End();
	}
